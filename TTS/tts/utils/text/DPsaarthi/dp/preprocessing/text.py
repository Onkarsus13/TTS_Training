...
class SequenceTokenizer:
    ...
    def __init__(self,
        ...
        print(f"token 2 idx => {self.token_to_idx} and idx 2 token => {self.idx_to_token}")
    ...
    def __call__(self, sentence: Iterable[str], language: str) -> List[int]:
        ...
        if not self.lowercase:
          sentence = [item for item in sentence.split(' ') for i in range(self.char_repeats)]
          #print(f"phoneme sent => {sentence}")
          space_info = []
          for i in range(len(sentence)):
            space_info.append(' ')

          sent = []
          for i, j in zip(sentence, space_info):
            sent.append(i)
            sent.append(j)
          sentence = sent[0:-1]

          #print(f"updated sent => {sentence}")
          ...
          #iters = [iter(sentence), iter(space_info)]
          #print(f"print iters => {iter(sentence)}")
          #sentence = list(iter.__next__() for iter in cycle(iters))

        else:
          sentence = [item for item in sentence for i in range(self.char_repeats)]
          #print(f"sentence from word => {sentence}")
