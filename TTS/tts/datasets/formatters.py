...
def mailabs(root_path, meta_files=None, ignored_speakers=None):
    ...
    """
    # print('PATHHHH...... SGfgwegyiowygi',root_path)
    speaker_regex = re.compile("by_book/(male|female)/(?P<speaker_name>[^/]+)/")
    if not meta_files:
        csv_files = glob(root_path + "/**/metadata.csv", recursive=True)
        # print('csv PATHHHHH', csv_files)
        
    else:
        csv_files = meta_files
    # ff = open("/root/Documents/Audio-Encoder-Pretraining-main/voice_coder/TTS_Custom/missing_files_paths/missing_files.txt", 'w')
    ...
    for csv_file in csv_files:
        ...
        # print("SNM",speaker_name_match)
        # if speaker_name_match is None:
        #     continue
        # speaker_name = speaker_name_match.group("speaker_name")
        speaker_name = folder.split('/')[-1]
        # print("Onkar fghwfghfh",speaker_name)
        # ignore speakers
