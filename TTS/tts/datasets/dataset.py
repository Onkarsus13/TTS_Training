...
class TTSDataset(Dataset):
    ...
    def get_token_ids(self, idx, text, lng_id, eng_p):
        if self.tokenizer.use_phonemes:
            token_ids = self.tokenizer.text_to_ids(text, lng_id, eng_p)#self.get_phonemes(idx, text)["token_ids"]
            # print(token_ids)
        else:
    ...
    def load_data(self, idx):
        ...
        # get token ids
        lng_id = item["language"]
        token_ids = self.get_token_ids(idx, item["text"], lng_id)
        # print("token_ids are :", token_ids)
    ...
    def collate_fn(self, batch):
        ...
        if isinstance(batch[0], collections.abc.Mapping):
            ...
            mel_lengths_adjusted = [
                m.shape[1] + (self_ids.outputs_per_step - (m.shape[1] % self.outputs_per_step))
