...
def synthesis(
    ...
    # lang = language_maping[language_id]
    # convert text to sequence of token IDs
    text_inputs = np.asarray(
        model.tokenizer.text_to_ids(text, language=language_maping, eng_p=eng_p),
        dtype=np.int32,
    )
    ...
    if not isinstance(style_mel, dict):
        ...
        if style_text is not None:
            style_text = np.asarray(
                model.tokenizer.text_to_ids(style_text, language=language_maping, eng_p=eng_p),
                dtype=np.int32,
            )
