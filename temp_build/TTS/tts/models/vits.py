...
class VitsDataset(TTSDataset):
    def __init__(self, model_args, *args, **kwargs):
        ...
        self.eng_p = Phonemizer.from_checkpoint('/root/Documents/Audio-Encoder-Pretraining-main/voice_coder/TTS/TTS/tts/utils/text/DPsaarthi/latest_model.pt')
    ...
    def __getitem__(self, idx):
        ...
        token_ids = self.get_token_ids(idx, item["text"], item['language'], self.eng_p)
    ...
    def collate_fn(self, batch):
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            wav = batch["wav"][i]
            # print(torch.FloatTensor(wav).shape)

            if torch.FloatTensor(wav).shape[0] == 2:
                wav = torch.FloatTensor(wav)[0:1, :]
...
class Vits(BaseTTS):
    ...
    def init_multispeaker(self, config: Coqpit):
        ...
        #     print('Its here', self.num_speakers)
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        if self.args.use_speaker_embedding and sid is not None:            
            ...
            # print('in use speaker', g.shape)
            # torch.sum(g).backward(retain_graph=True)
    ...
    def load_checkpoint(
        ...
        if hasattr(self, "emb_g") and state["model"]["emb_g.weight"].shape != self.emb_g.weight.shape:
            # print(self.emb_g.weight.shape, state["model"]["emb_g.weight"].shape)
            ...
            emb_g = state["model"]["emb_g.weight"] #num_sp, emd_dim
            new_row = torch.randn(num_new_speakers, emb_g.shape[1])
            emb_g = torch.cat([emb_g, new_row], axis=0) #num+1, em_dims
        ...
        if self.args.use_language_embedding and self.language_manager:
            if state['model']['emb_l.weight'].shape != self.emb_l.weight.shape:
                print('New Language id is creating......')
                new_lang = self.emb_l.weight.shape[0] - state['model']['emb_l.weight'].shape[0]
                emb_l = state['model']['emb_l.weight']
                new_lan_row = torch.randn(new_lang, emb_l.shape[1])
                emb_l = torch.cat([emb_l, new_lan_row], axis=0)
                state['model']['emb_l.weight'] = emb_l

            print("......Weights loaded with languege ids......")
